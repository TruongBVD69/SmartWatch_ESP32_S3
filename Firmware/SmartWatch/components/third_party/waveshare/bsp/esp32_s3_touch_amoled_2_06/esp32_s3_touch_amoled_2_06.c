...
static bool i2s_tx_enabled = false;
static bool i2s_rx_enabled = false;
static const audio_codec_data_if_t *i2s_tx_data_if = NULL;
static const audio_codec_data_if_t *i2s_rx_data_if = NULL;
...
esp_err_t bsp_audio_init(const i2s_std_config_t *i2s_config)
{
    ...
    i2s_tx_data_if = audio_codec_new_i2s_data(&tx_i2s_cfg);
    BSP_NULL_CHECK_GOTO(i2s_tx_data_if, err);

    audio_codec_i2s_cfg_t rx_i2s_cfg = {
        .port = CONFIG_BSP_I2S_NUM,
        .rx_handle = i2s_rx_chan,
    };
    i2s_rx_data_if = audio_codec_new_i2s_data(&rx_i2s_cfg);
    BSP_NULL_CHECK_GOTO(i2s_rx_data_if, err);
...
err:
    if (i2s_tx_data_if)
    {
        audio_codec_delete_data_if(i2s_tx_data_if);
        i2s_tx_data_if = NULL;
    }
    if (i2s_rx_data_if)
    {
        audio_codec_delete_data_if(i2s_rx_data_if);
        i2s_rx_data_if = NULL;
    }
    if (i2s_tx_chan)
    {
...
esp_codec_dev_handle_t bsp_audio_codec_speaker_init(void)
{
    ...
    BSP_NULL_CHECK(speaker_ctrl_if, NULL);
    ...
    BSP_NULL_CHECK(speaker_codec_if, NULL);

    esp_codec_dev_cfg_t codec_dev_cfg = {
        .dev_type = ESP_CODEC_DEV_TYPE_OUT,
        .codec_if = speaker_codec_if,
        .data_if = i2s_tx_data_if,
    };
...
esp_err_t bsp_touch_new(const bsp_touch_config_t *config, esp_lcd_touch_handle_t *ret_touch)
{
    ...
    return esp_lcd_touch_new_i2c_ft3168(tp_io_handle, &tp_cfg, ret_touch);
}
